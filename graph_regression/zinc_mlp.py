...
for lr in LR:
    for hidden_dim in HIDDEN_DIM:
        for n_layers in N_LAYERS:
            ...
            model = GIN(args.n_gnn_layers, 21, hidden_dim, n_layers, 1, args.dropout, True).to(device)
...
for run in range(10):
    ...
    model = GIN(args.n_gnn_layers, 21, best_hyperparams['hidden_dim'], best_hyperparams['n_layers'], 1, args.dropout, True).to(device)
