...
parser.add_argument('--gnn-type', default='GCN', choices=['GIN', 'GCN'], help='GNN model')
parser.add_argument('--model-type', default='FASTKAN', choices=['MLP', 'KAN', 'FASTKAN'], help='Update model')
...
def train_model_with_parameters(params, train_loader, val_loader, test_loader=None):
    if args.model_type == 'MLP' and args.gnn_type == 'GIN':
        model = GIN(11, 4, args.num_gnn_layers, params['hidden_dim'], params['hidden_layers'], 12, params['dropout'], False).to(device)
    elif args.model_type == 'MLP' and args.gnn_type == 'GCN':
        model = GCN(11, args.num_gnn_layers, params['hidden_dim'], 12, params['dropout'], False).to(device)
    elif args.model_type == 'KAN' and args.gnn_type == 'GIN':
        model = KAGIN(11, 4, args.num_gnn_layers, params['hidden_dim'], params['hidden_layers'], params['grid_size'], params['spline_order'], 12, params['dropout'], False).to(device)
    elif args.model_type == 'KAN' and args.gnn_type == 'GCN':
        model = KAGCN(11, args.num_gnn_layers, params['hidden_dim'], params['grid_size'], params['spline_order'], 12, params['dropout'], False).to(device)
    elif args.model_type == 'FASTKAN' and args.gnn_type == 'GIN':
        model = FASTKAGIN(11, 4, args.num_gnn_layers, params['hidden_dim'], params['hidden_layers'], params['grid_size'], 12, params['dropout'], False).to(device)
    elif args.model_type == 'FASTKAN' and args.gnn_type == 'GCN':
        model = FASTKAGCN(11, args.num_gnn_layers, params['hidden_dim'], params['grid_size'], 12, params['dropout'], False).to(device)
    print(model)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        model.eval()
        val_loss = torch.zeros([1, 12]).to(device)
        for data in val_loader:
            data = data.to(device)
            val_loss += ((data.y * std - model(data) * std).abs() / std).sum(dim=0)
        ...
        if best_val_loss >= val_loss:
            ...
            if test_loader is not None:
                test_loss = torch.zeros([1, 12]).to(device)
                for data in test_loader:
                    data = data.to(device)
                    test_loss += ((data.y * std - model(data) * std).abs() / std).sum(dim=0)
