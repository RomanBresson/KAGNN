...
for lr in LR:
    for hidden_dim in HIDDEN_DIM:
        for n_layers in N_LAYERS:
            ...
            model = GIN(args.n_gnn_layers, dataset.num_features, hidden_dim, n_layers, 12, args.dropout).to(device)
...
for _ in range(10):
    ...
    model = GIN(args.n_gnn_layers, dataset.num_features, best_hyperparams['hidden_dim'], best_hyperparams['n_layers'], 12, args.dropout).to(device)
