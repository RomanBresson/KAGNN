...
from models import FASTKAGIN, FASTKAGCN, FASTKAGAT
...
def train_model_with_parameters(params, train_loader, val_loader, test_loader=None):
    ...
    if args.model_type == 'GIN':
        ...
        model = FASTKAGCN(layers_per_dataset[args.dataset], train_loader.dataset.num_features, hidden_dim, train_loader.dataset.num_classes, grid_size, dropout).to(device)
    elif args.model_type == 'GAT':
        model = FASTKAGAT(layers_per_dataset[args.dataset], train_loader.dataset.num_features, hidden_dim, train_loader.dataset.num_classes, grid_size, dropout, heads=args.heads).to(device)
    print(model)
...
def objective(trial, train_loader, val_loader):
    ...
    params = {'lr': lr, 'hidden_layers':hidden_layers, 'grid_size':grid_size, 'dropout':dropout, 'hidden_dim':hidden_dim, 'model_type':args.model_type}
    best_val_loss = train_model_with_parameters(params, train_loader, val_loader)
    return best_val_loss

log_file = f'logs/FASTKAN_{args.dataset}_{args.model_type}'
